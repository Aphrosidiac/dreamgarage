<template>
  <div class="max-w-3xl">
    <div v-if="loading" class="text-dark-400">Loading profile...</div>
    ...
    <template v-else-if="profile">
      ...
      <div class="bg-dark-900 border border-dark-800 rounded-xl p-6 mb-6">
        <div class="flex items-center gap-5 mb-6">
          <div class="w-16 h-16 bg-gold-500/20 rounded-full flex items-center justify-center">
            <span class="text-gold-500 text-xl font-bold">{{ userInitials }}</span>
          </div>
          <div>
            <h2 class="text-xl font-semibold text-dark-100">{{ profile.name }}</h2>
            <p class="text-dark-400 text-sm">{{ profile.email }}</p>
            <BaseBadge :color="profile.role === 'ADMIN' ? 'gold' : 'gray'" class="mt-1">{{ profile.role }}</BaseBadge>
          </div>
        </div>

        <div class="grid sm:grid-cols-2 gap-4 pt-4 border-t border-dark-800">
          <div>
            <p class="text-dark-500 text-xs uppercase tracking-wider mb-1">Branch</p>
            <p class="text-dark-200 text-sm">{{ profile.branch?.name }}</p>
            <p class="text-dark-400 text-xs">{{ profile.branch?.code }}</p>
          </div>
          <div>
            <p class="text-dark-500 text-xs uppercase tracking-wider mb-1">Member Since</p>
            <p class="text-dark-200 text-sm">{{ new Date(profile.createdAt).toLocaleDateString('en-MY', { year: 'numeric', month: 'long', day: 'numeric' }) }}</p>
          </div>
        </div>
      </div>

      <!-- Branch Settings (Admin only) -->
      <div v-if="profile.role === 'ADMIN'" class="bg-dark-900 border border-dark-800 rounded-xl p-6 mb-6">
        <h3 class="text-sm font-semibold text-dark-200 uppercase tracking-wider mb-4">Branch / Company Details</h3>
        <form @submit.prevent="handleUpdateBranch" class="space-y-4">
          <div class="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-dark-200 mb-1.5">Branch Code</label>
              <p class="text-dark-400 text-sm py-2.5 px-3 bg-dark-800 border border-dark-700 rounded-lg opacity-60">{{ profile.branch?.code }}</p>
            </div>
          </div>
      ...
      <!-- Edit Profile -->
      <div class="bg-dark-900 border border-dark-800 rounded-xl p-6 mb-6">
        <h3 class="text-sm font-semibold text-dark-200 uppercase tracking-wider mb-4">Edit Profile</h3>
      ...
      <!-- Change Password -->
      <div class="bg-dark-900 border border-dark-800 rounded-xl p-6 mb-6">
        <h3 class="text-sm font-semibold text-dark-200 uppercase tracking-wider mb-4">Change Password</h3>
      ...
      <!-- Danger Zone -->
      <div class="bg-dark-900 border border-red-900/30 rounded-xl p-6">
        <h3 class="text-sm font-semibold text-red-400 uppercase tracking-wider mb-2">Session</h3>
        <p class="text-dark-400 text-sm mb-4">Sign out from this device.</p>
