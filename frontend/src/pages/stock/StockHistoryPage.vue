  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-lg font-semibold text-dark-100">Stock History</h2>
          <p v-if="item" class="text-dark-400 text-sm">
            <span class="font-mono text-gold-500">{{ item.itemCode }}</span> — {{ item.description }}
            <span class="ml-2 text-dark-500">(Current: {{ item.quantity }} {{ item.uom }})</span>
          </p>
        </div>
      </div>
    ...
    <BaseTable :columns="columns" :data="stock.history" :loading="stock.historyLoading" empty-text="No stock movements recorded.">
      ...
      <template #cell-previousQty="{ row }">
        <span class="text-dark-400">{{ row.previousQty }}</span>
        <span class="text-dark-600 mx-1">→</span>
        <span class="text-dark-200 font-medium">{{ row.newQty }}</span>
      </template>
      <template #cell-reason="{ row }">
        <span class="text-dark-300">{{ row.reason }}</span>
        <RouterLink v-if="row.document" :to="`/app/documents/${row.documentId}`" class="text-gold-500 text-xs ml-1 hover:text-gold-400">
    ...
    <BaseModal v-model="showAdjustModal" title="Adjust Stock" size="sm">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-dark-200 mb-1.5">Type</label>
          <div class="flex gap-2">
            <button
              type="button"
              @click="adjustForm.type = 'add'"
              :class="['flex-1 py-2 rounded-lg text-sm font-medium border transition-colors', adjustForm.type === 'add' ? 'bg-green-500/10 border-green-500 text-green-400' : 'border-dark-700 text-dark-400 hover:border-dark-600']"
            >
              + Add Stock
            </button>
            <button
              ...
              :class="['flex-1 py-2 rounded-lg text-sm font-medium border transition-colors', adjustForm.type === 'remove' ? 'bg-red-500/10 border-red-500 text-red-400' : 'border-dark-700 text-dark-400 hover:border-dark-600']"
            >
              - Remove Stock
            </button>
          </div>
