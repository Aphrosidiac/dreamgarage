  ...
  <div class="max-w-7xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-dark-400 mt-1">Admin-only activity trail</p>
      </div>
    </div>
    ...
    <BaseTable :columns="columns" :data="rows" :loading="loading" empty-text="No audit entries match these filters.">
      <template #cell-createdAt="{ value }">
        <span class="text-dark-400 text-xs font-mono">{{ fmt(value) }}</span>
      </template>
      ...
      <template #cell-path="{ row }">
        <span class="font-mono text-xs text-dark-300">{{ row.method }} {{ row.path }}</span>
      </template>
      <template #actions="{ row }">
        <button v-if="row.changes" @click="openChanges(row as AuditRow)" class="p-1.5 text-dark-400 hover:text-gold-500 transition-colors" title="View details">
          <Eye class="w-4 h-4" />
        </button>
      </template>
    </BaseTable>
    ...
    <BaseModal v-model="showChanges" title="Audit details" size="lg">
      <pre class="text-xs font-mono text-dark-200 bg-dark-900 p-4 rounded-lg overflow-auto max-h-[60vh]">{{ selectedChanges }}</pre>
    </BaseModal>
  </div>
</template>
