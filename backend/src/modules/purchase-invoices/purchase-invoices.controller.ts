...
export async function finalizeInvoice(
  ...
  // Track which stock items are tyres for DOT recalculation
  const tyreStockIds = new Set<string>()
  ...
  await prisma.$transaction(async (tx: any) => {
    for (const item of invoice.items) {
      ...
      const stockItem = await tx.stockItem.findUnique({
        ...
        select: { id: true, quantity: true, isTyre: true },
      })
      ...
      // Handle TyreDOT if applicable
      if (item.dotCode && stockItem.isTyre) {
        tyreStockIds.add(item.stockItemId)

        await tx.tyreDOT.upsert({
          where: {
            stockItemId_dotCode: {
    ...
    // Recalculate tyre stock quantities from DOT batches
    for (const stockId of tyreStockIds) {
      const dotBatches = await tx.tyreDOT.findMany({
        where: { stockItemId: stockId, isActive: true },
        select: { quantity: true },
      })
      const totalFromDots = dotBatches.reduce((sum: number, d: any) => sum + d.quantity, 0)

      await tx.stockItem.update({
        where: { id: stockId },
        data: { quantity: totalFromDots },
      })
    }
